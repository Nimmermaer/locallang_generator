...
namespace Mblunck\LocallangGenerator\Command;

use Mblunck\LocallangGenerator\Extractor\ExtractorInterface;
use Mblunck\LocallangGenerator\Reader\TranslationReaderInterface;
use Mblunck\LocallangGenerator\Writer\TranslationWriterInterface;
...
class TranslationUpdateCommand extends Command
{
    ...
    private string $defaultTransPath = 'Resources/Private/Language';
    private string $defaultViewsPath = '';
    private array $transPaths = [];
    private array $codePath = [];
    ...
    protected function configure(): void
    {
        $this
            ->setDefinition([
                ...
                new InputOption('format', null, InputOption::VALUE_OPTIONAL, 'Override the default output format',
                    'xlf12'),
                new InputOption('dump-messages', null, InputOption::VALUE_NONE,
                ...
                new InputOption('sort', null, InputOption::VALUE_OPTIONAL,
                    'Return list of messages sorted alphabetically (only works with --dump-messages)', 'asc'),
                new InputOption('as-tree', null, InputOption::VALUE_OPTIONAL,
                    'Dump the messages as a tree-like structure: The given value defines the level where to switch to inline YAML'),
            ])
            ->setHelp(<<<'EOF'
The <info>%command.name%</info> command extracts translation strings from templates
of a given extension or the default translations directory. It can display them or merge
the new ones into the translation files.

When new translation strings are found, it can automatically add a prefix to the translation
message.

Example running against an Extension (YourExtensionKey):

  <info>php %command.full_name% --dump-messages en YourExtensionKey</info>
  <info>php %command.full_name% --force --prefix="new_" fr YourExtensionKey</info>
...
You can sort the output with the <comment>--sort</> flag:
    ...
    <info>php %command.full_name% --dump-messages --sort=asc en YourExtensionKey</info>
    <info>php %command.full_name% --dump-messages --sort=desc fr</info>

You can dump a tree-like structure using the YAML format with the <comment>--as-tree</> flag:

    <info>php %command.full_name% --force --format=yaml --as-tree=3 en YourExtensionKey</info>
